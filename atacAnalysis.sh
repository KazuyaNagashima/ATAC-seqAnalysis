...
echo '以上の設定でATAC-seq解析を行いますか？[y/n]'
read qestion
if [ $qestion = 'y' ]; then
  echo 'プログラムを実行します'
else
...
mkdir ${dir}/${sample}_${sra}
cd ${dir}/${sample}_${sra}


#pfastq-dumpでSRAをfastqに変換
if [ $ends = 'single' ]; then
   pfastq-dump --threads 8  --gzip -s ../sra/${sra}.sra
elif [ $ends = 'pair' ]; then
   #Pair Endsの時は--split-filesをつける
   pfastq-dump --threads 8 --gzip --split-files -s ../sra/${sra}.sra
else
...
mkdir bowtie2
if [ $ends = 'single' ]; then
   bowtie2 -p 8 -t -q -N 1 -L 25 -X 2000 --no-discordant -x /Users/shigenseigyo/Desktop/reference/Mus_musculus/UCSC/${ref}/Sequence/Bowtie2Index/genome -U ./fastp_reports/fastp_${sra}.fastq -S ./bowtie2/${sra}.sam
   rm ./fastp_reports/fastp_${sra}.fastq
else
   bowtie2 -p 8 -t -q -N 1 -L 25 -X 2000 --no-mixed --no-discordant -x /Users/shigenseigyo/Desktop/reference/Mus_musculus/UCSC/${ref}/Sequence/Bowtie2Index/genome -1 ./fastp_reports/fastp_${sra}R1.fastq -2 ./fastp_reports/fastp_${sra}R2.fastq -S ./bowtie2/${sra}.sam
...
mkdir deeptools
bamCoverage -p 6 -b ./bowtie2/${sra}.drm.bam -o ./deeptools/${sra}.bw -of bigwig --binSize 100 -e 250 --scaleFactor 1
...
mkdir macs2
if [ $ends = 'single' ]; then
   macs2 callpeak -t ./bowtie2/${sra}.drm.bam -n ${sra} -g mm --outdir macs2 -q 0.01 --broad --nolambda --SPMR --nomodel -B
   ...
   macs2 callpeak -t ./bowtie2/${sra}.drm.bam -n ${sra} -f BAMPE -g mm --outdir macs2 -q 0.01 --broad --nolambda --SPMR --nomodel -B
fi
